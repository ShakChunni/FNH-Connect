...
import AdmissionHospitalSearch from "./AdmissionHospitalSearch";
import AdmissionHospitalDetails from "./AdmissionHospitalDetails";
import { useAdmissionHospitalData } from "../../../stores";

interface AdmissionHospitalInformationProps {
  readonly?: boolean;
}

const AdmissionHospitalInformation: React.FC<
  AdmissionHospitalInformationProps
> = ({ readonly = false }) => {
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-blue-700 text-[11px] sm:text-xs font-medium leading-tight transition-colors duration-300 mt-1">
              {readonly ? "Referring hospital details" : getDescription()}
            </p>
          </div>
        </div>
      </div>

      {!readonly && <AdmissionHospitalSearch />}
      <AdmissionHospitalDetails readonly={readonly} />
    </div>
  );
};

export default React.memo(AdmissionHospitalInformation);
