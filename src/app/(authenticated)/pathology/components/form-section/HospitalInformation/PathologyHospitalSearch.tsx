...
const PathologyHospitalSearch: React.FC = () => {
  ...
  // Sync searchQuery with store - also handles reset
  useEffect(() => {
    // If store name is different from local, update local
    if (hospitalData.name !== searchQuery) {
      setSearchQueryState(hospitalData.name || "");
    }
  }, [hospitalData.name]); // Intentionally not including searchQuery to avoid loops

  const { data: hospitals = [], isLoading: loading } =
  ...
  // Sync status with store - handles reset
  useEffect(() => {
    if (hospitalData.id) {
      ...
      // Store was reset - clear status
      setHospitalStatus("");
    }
  }, [hospitalData.id, hospitalData.name]);
...
export default PathologyHospitalSearch;
