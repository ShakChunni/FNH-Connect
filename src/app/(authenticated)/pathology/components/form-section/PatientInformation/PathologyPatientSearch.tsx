...
const PathologyPatientSearch: React.FC = () => {
  ...
  useEffect(() => {
    // If store fullName is different from local, update local
    if (patientData.fullName !== searchQuery) {
      setSearchQueryState(patientData.fullName || "");
    }
  }, [patientData.fullName]); // Intentionally not including searchQuery to avoid loops
  ...
  const handleSelectPatient = (patient: PathologyPatientBasic) => {
    ...
    // Set guardian data - autofill from guardian fields in patient record
    setGuardianData({
