 ...
 * Searchable dropdown for selecting medicine groups/categories with inline add capability
 */
...
interface GroupSearchProps {
  value: number | null;
  displayValue: string;
  onChange: (group: MedicineGroup | null) => void;
...
export const GroupSearch: React.FC<GroupSearchProps> = ({
  ...
  }, [displayValue, isOpen, searchQuery]);

  // Fetch groups
  const { data: groups = [], isLoading } = useFetchMedicineGroups(true);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newQuery.length >= 1 || groups.length > 0) {
  ...
  const handleSelectGroup = (group: MedicineGroup) => {
    onChange(group);
  ...
  const showResults = isOpen && filteredGroups.length > 0;
    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          onFocus={() => {
            if (searchQuery.length >= 1 || groups.length > 0) {
              updateDropdownPosition();
              setIsOpen(true);
            }
          }}
        ...
        {!value && searchQuery && !disabled && (
          <div className="absolute right-3 top-1/2 -translate-y-1/2">
            <Check className="w-4 h-4 text-purple-500 opacity-0" />
          </div>
        )}
      </div>
            ...
            >
                ...
                <div className="max-h-[200px] overflow-y-auto">
                  {filteredGroups.map((group) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left hover:bg-purple-50 transition-colors flex items-center gap-3 cursor-pointer ${
                        value === group.id ? "bg-purple-50" : ""
                      }`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {group._count?.medicines !== undefined && (
                          <p className="text-xs text-gray-500">
                            {group._count.medicines} medicines
                          </p>
                        )}
                      </div>
                      {value === group.id && (
                ...
                <div className="p-4">
                  ...
                  {onAddNew && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors cursor-pointer"
                    >
                      <PlusCircle className="w-4 h-4 text-purple-600" />
                      <span className="text-sm font-semibold text-purple-700">
                        Add &quot;{searchQuery}&quot; as new group
                      </span>
                    </button>
                  )}
                </div>
              ...
              {showResults && onAddNew && searchQuery.trim() && (
                <div className="border-t border-gray-100 p-2">
                  <button
                    onClick={handleAddNew}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 hover:bg-purple-50 rounded-lg transition-colors cursor-pointer"
                  >
                    <PlusCircle className="w-4 h-4 text-purple-600" />
