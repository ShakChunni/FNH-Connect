...
export const DoctorFilter: React.FC = () => {
  ...
  const doctorId = useFilterStore((state) => state.filters.doctorId);
  const setDoctorId = useFilterStore((state) => state.setDoctorId);
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <span
          className={doctorId ? "text-gray-900 font-medium" : "text-gray-500"}
        >
          {displayLabel}
        </span>
        <ChevronDown
            ...
            >
              <div className="flex flex-col">
                ...
                {doctor.specialization && (
                  <span
                    className={`text-xs ${
                      doctorId === doctor.id ? "text-gray-200" : "text-gray-400"
                    }`}
                  >
                    {doctor.specialization}
                  </span>
