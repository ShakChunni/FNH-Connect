...
const InvestigationsTable: React.FC<InvestigationsTableProps> = ({
  tableData = [],
  customOptions,
  onEdit,
  isLoading = false,
  ...
  const [selectedPatient, setSelectedPatient] =
    useState<InfertilityTestData | null>(null);
  const [isOverviewOpen, setIsOverviewOpen] = useState(false);

  // Horizontal drag-to-scroll for better UX
  const dragScroll = useHorizontalDragScroll<HTMLDivElement>();

  const handlePatientClick = (patient: InfertilityTestData) => {
    setSelectedPatient(patient);
    setIsOverviewOpen(true);
  };

  // Sorting logic (local for the current page)
    ...
    if (sortConfig !== null) {
      sortableData.sort((a, b) => {
        ...
        if (
          ...
          const dateA = a[key as keyof InfertilityTestData]
            ? new Date(a[key as keyof InfertilityTestData] as string).getTime()
            : 0;
          const dateB = b[key as keyof InfertilityTestData]
            ? new Date(b[key as keyof InfertilityTestData] as string).getTime()
            : 0;
          return (dateA - dateB) * dir;
        }

        if (
          typeof a[key as keyof InfertilityTestData] === "number" &&
          typeof b[key as keyof InfertilityTestData] === "number"
        ) {
          return (
            ((a[key as keyof InfertilityTestData] as number) -
              (b[key as keyof InfertilityTestData] as number)) *
            dir
          );
        }

        const valA = (a[key as keyof InfertilityTestData] || "")
          .toString()
          .toLowerCase();
        const valB = (b[key as keyof InfertilityTestData] || "")
          .toString()
          .toLowerCase();
