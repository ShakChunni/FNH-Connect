...
interface PatientOverviewProps {
  isOpen: boolean;
  onClose: () => void;
  patient: InfertilityTestData | null;
...
const PatientOverview: React.FC<PatientOverviewProps> = ({
  ...
  const handlePrint = () => {
    generateInfertilityTestReceipt(patient, user?.fullName || "Staff");
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2 py-1 bg-slate-50 border border-slate-100 rounded-md">
            ...
            <span className="text-[10px] font-bold text-gray-600">
              {formatDateWithTime(patient.testDate).date}
            </span>
            <span className="text-[9px] font-medium text-gray-400">
              {formatDateWithTime(patient.testDate).time}
            </span>
          </div>
          <div
