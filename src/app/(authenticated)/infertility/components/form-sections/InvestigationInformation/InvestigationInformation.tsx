...
const InvestigationInformation: React.FC<InvestigationInformationProps> = ({
  ...
  const {
    testCharge,
    // paidAmount, // Not used directly in rendering input value (we use testInfo.paidAmount) to avoid stale closure if we destructured
  ...
  useEffect(() => {
    ...
    const storeVal = testInfo.discountValue;
  ...
  const handleTestsChange = (tests: string[], total: number) => {
    // This helper both updates specific fields via setInfertilityTestInfo AND sets the charge for recalc
    setInfertilityTestInfo({
      ...testInfo,
      selectedTests: tests,
  ...
  const handleDiscountInputChange = (
    ...
    if (val === "") {
      setDiscountInput("");
      setDiscount(testInfo.discountType as "percentage" | "value", null);
    } else {
      const numVal = Number(val);
      setDiscountInput(numVal);
      setDiscount(testInfo.discountType as "percentage" | "value", numVal);
    }
  };
    ...
    <div id="pathology" className="mt-2 sm:mt-0 mb-6 sm:mb-8 md:mb-10">
      ...
      <div className="mb-6 pt-6 border-t border-gray-200">
        ...
        <div>
          ...
          <input
            type="text"
            className={inputClassName(testInfo.testDate || "", true)}
            value={
              testInfo.testDate
                ? (() => {
                    const d = new Date(testInfo.testDate);
          ...
          <div>
            ...
            <div className="relative flex items-stretch rounded-lg border-2 border-gray-300 bg-white overflow-hidden focus-within:border-green-600 focus-within:ring-2 focus-within:ring-green-100 transition-all duration-300 h-12 md:h-14">
              ...
              <NumberInput
                ...
                min="0"
                max={
                  testInfo.discountType === "percentage"
                    ? 100
                    : testInfo.testCharge
                }
              />
              ...
              <div className="mt-2 p-2.5 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-green-600">
                    ৳{(testInfo.discountAmount || 0).toLocaleString()}
                    {testInfo.discountType === "percentage" &&
                      discountInput !== "" &&
                      discountInput > 0 && (
            ...
            <div>
              ...
              <div className="relative">
                ...
                <NumberInput
                  ...
                  value={testInfo.paidAmount || ""}
                  onChange={handlePaymentChange}
                  placeholder="Enter payment amount"
                  min="0"
                  max={testInfo.grandTotal}
                />
              </div>
              <p className="text-xs text-gray-500 mt-1">
...
export default React.memo(InvestigationInformation);
