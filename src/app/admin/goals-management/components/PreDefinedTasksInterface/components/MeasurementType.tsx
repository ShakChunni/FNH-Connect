...
const MeasurementType: React.FC<MeasurementTypeProps> = ({
  selectedValue,
  onSelect,
  error,
}) => {
  ...
  const options = useMemo(
    () => [
      { label: "Quantity", value: "QUANTITY_ONLY" },
      { label: "Time", value: "TIME_ONLY" },
            ...
            {options.map((option) => (
              <div
                ...
                className={`cursor-pointer px-4 py-2 sm:py-3 text-xs sm:text-sm transition-colors duration-150 hover:bg-blue-950 hover:text-white ${
                  selectedValue === option.value ? "bg-blue-900 text-white" : ""
                }`}
              >
                {option.label}
              </div>
...
export default React.memo(MeasurementType);
