...
const OrganizationInformation: React.FC<OrganizationInformationProps> = ({
  ...
  // Allow fetching whenever input is focused and not selected, or when searchQuery is present and not selected
  const shouldFetch =
    !localData.id && (isInputFocused || searchQuery.length >= 1);
  const { organizations, loading } = useFetchOrganizationInformation(
    searchQuery,
    shouldFetch
  );
  ...
  useEffect(() => {
    if (value) {
      ...
      setOrgStatus(value.id ? "existing" : "");
      setOrgTouched(false);
      setIsNewMode(false);
  ...
  const handleSelectOrganization = useCallback(
    ...
    [onDropdownToggle, localData.campaignName]
  );
  ...
  const handleClearSelection = () => {
    const campaignName = localData.campaignName;
    setLocalData({
      id: null,
      name: "",
      website: "",
  ...
  const handleScroll = useCallback(
    (event: Event) => {
      ...
      if (
        ...
        orgInputRef.current?.contains(target)
      ) {
        return;
      }
  ...
  const isLocked = isOrgSelected && !isNewMode;
  ...
  const dropdownContent = (
    <AnimatePresence>
      {(isDropdownOpen || isInputFocused) && !isOrgSelected && (
        <motion.div
          ref={dropdownRef}
        ...
        >
          ...
          >
            ...
            {!loading && shouldFetch && searchQuery.length === 0 && (
            ...
            {!loading && shouldFetch && searchQuery.length > 1 && (
              <div
                onMouseDown={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
      ...
      <div className="mb-3 sm:mb-4 relative">
        ...
        <div className="relative flex items-center">
          <input
            ...
            type="text"
            className={
              inputClassName(localData.name, isLocked) + " pr-44 sm:pr-56"
            }
            value={searchQuery}
            onChange={isLocked ? undefined : handleInputChange}
            onBlur={isLocked ? undefined : handleInputBlur}
            onFocus={() => {
      ...
      <div className="mb-3 sm:mb-4 relative">
        ...
        <div className="relative">
          <IndustryDropdown
            ...
            disabled={isLocked && autofilledFields.industry}
          />
          {isOrgSelected && !isNewMode && autofilledFields.industry && (
      ...
      <div className="mb-3 sm:mb-4 relative">
        ...
        <div className="relative">
          <CountryDropdown
            ...
            disabled={isLocked && autofilledFields.location}
          />
          {isOrgSelected && !isNewMode && autofilledFields.location && (
            <div className="hidden sm:flex absolute right-3 top-1/2 -translate-y-1/2">
              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold border border-blue-200 shadow-sm pointer-events-none">
        ...
        <div className="relative">
          <input
            ...
            className={inputClassName(
              localData.website,
              isLocked && autofilledFields.website
            )}
            value={localData.website}
            onChange={(e) => handleLocalDataChange("website", e.target.value)}
            placeholder="Enter organization website"
            disabled={isLocked && autofilledFields.website}
          />
          {isOrgSelected && !isNewMode && autofilledFields.website && (
            <div className="hidden sm:flex absolute right-3 top-1/2 -translate-y-1/2">
              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold border border-blue-200 shadow-sm pointer-events-none">
