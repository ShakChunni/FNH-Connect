...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const data = {
      ...
      source_organization: sourceTable === "REDACTED" ? "REDACTED" : "MI",
      createdAt: klTime,
      updatedAt: null,
      createdBy: username,
    };
    ...
    if (sourceTable === "REDACTED") {
      createdRecord = await prisma.REDACTED_monthly_report.create({ data });
    ...
    const displayId =
      sourceTable === "MI"
        ? `REDACTED-${createdRecord.id}`
        : `MV-${createdRecord.id}`;
    const organizationDisplayName = sourceTable === "MI" ? "REDACTED" : "REDACTED";
