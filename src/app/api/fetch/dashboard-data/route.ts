...
export async function POST(req: NextRequest) {
  try {
    ...
    let REDACTEDReports: any[] = [];
    let miReports: any[] = [];
    let previousREDACTEDReports: any[] = [];
    ...
    if (hasDateRange && !hasValidSearch) {
      ...
      [REDACTEDReports, miReports, previousREDACTEDReports, previousMiReports] =
        await Promise.all([
          tableSelector === "All" || tableSelector === "REDACTED"
            ? prisma.REDACTED_monthly_report.findMany({ where: whereClause })
            : Promise.resolve([]),
          ...
          tableSelector === "All" || tableSelector === "REDACTED"
            ? prisma.REDACTED_monthly_report.findMany({
      ...
      [REDACTEDReports, miReports] = await Promise.all([
        tableSelector === "All" || tableSelector === "REDACTED"
          ? prisma.REDACTED_monthly_report.findMany({ where: whereClause })
          : Promise.resolve([]),
    ...
    const filteredData = [...REDACTEDReports, ...miReports].sort(
      (a, b) =>
