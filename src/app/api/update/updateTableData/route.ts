...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const originalDisplayId =
      originalSourceTable === "REDACTED" ? `MV-${id}` : `REDACTED-${id}`;

    let originalRecord;
    if (originalSourceTable === "REDACTED") {
      originalRecord = await prisma.REDACTED_monthly_report.findUnique({
        where: { id },
      });
    } else {
    ...
    if (!originalRecord) {
      const tableName =
        originalSourceTable === "REDACTED"
          ? "REDACTED_monthly_report"
          : "mi_monthly_report";
      throw new Error(
    ...
    const originalSourceOrg =
      ...
      (originalSourceTable === "REDACTED" ? "REDACTED" : "MI");
    ...
    if (originalSourceTable !== sourceTable) {
      ...
      if (sourceTable === "REDACTED") {
        updatedRecord = await prisma.REDACTED_monthly_report.create({
          data: newData,
        });
      } else {
      ...
      if (originalSourceTable === "REDACTED") {
        await prisma.REDACTED_monthly_report.delete({
          where: { id },
        });
      } else {
      ...
      const newDisplayId =
        sourceTable === "REDACTED"
          ? `MV-${updatedRecord.id}`
          : `REDACTED-${updatedRecord.id}`;
      action = "TRANSFER";
      sourceId = updatedRecord.id;
      displayId = newDisplayId;
      organizationDisplayName = sourceTable === "MI" ? "REDACTED" : "REDACTED";
      logDescription = `Transferred record ${clientName} from ${
        originalSourceTable === "MI" ? "REDACTED" : "REDACTED"
      } to ${
        sourceTable === "MI" ? "REDACTED" : "REDACTED"
      ...
      if (sourceTable === "REDACTED") {
        updatedRecord = await prisma.REDACTED_monthly_report.update({
          where: { id },
          data: updateData,
        });
      } else {
      ...
      displayId = sourceTable === "MI" ? `REDACTED-${id}` : `MV-${id}`;
      action = "UPDATE";
      sourceId = id;
      organizationDisplayName = sourceTable === "MI" ? "REDACTED" : "REDACTED";
