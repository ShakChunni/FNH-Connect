...
export async function PUT(req: NextRequest) {
  try {
    const { id, name, type, notes, meetingType, username } = await req.json();

    if (!id || !name || !type || !username) {
      return NextResponse.json(
        { error: "Missing required fields: id, name, type, or username" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Error updating ad hoc task" },
      { status: 500 }
    );
  }
}
...
async function logActivity(
  ...
  const { userId, username, id, formattedName, klTime } = params;
  ...
  try {
    ...
    await prisma.activityLog.create({
      data: {
        userId,
        username,
        action: "UPDATE_AD_HOC_TASK",
        description: `${username} updated ad-hoc task ${id} - ${formattedName}`,
        source_id: id,
