...
export async function GET(request: NextRequest) {
  try {
    ...
    // Default to today's date in Bangladesh time (UTC+6)
    const BDT_OFFSET_MS = 6 * 60 * 60 * 1000;
    const nowBDT = new Date(Date.now() + BDT_OFFSET_MS);
    const today = `${nowBDT.getUTCFullYear()}-${String(
      nowBDT.getUTCMonth() + 1
    ).padStart(2, "0")}-${String(nowBDT.getUTCDate()).padStart(2, "0")}`;
